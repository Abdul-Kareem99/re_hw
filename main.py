...
def fix_number(phones):
    ...
    for index, el in enumerate(phones[1:]):
        el[5] = new_phones[index]
    return phones


def fix_names(names):
    for i in names[1:]:
        for index, el in enumerate(i[:3]):
            result = re.findall(r'\w+\S', el)
            if len(result) == 3:
                i[0] = result[0]
                i[1] = result[1]
                i[2] = result[2]
            elif len(result) == 2 and index == 0:
                i[0] = result[0]
                i[1] = result[1]
            elif len(result) == 2 and index == 1:
                i[1] = result[0]
                i[2] = result[1]
    return names
